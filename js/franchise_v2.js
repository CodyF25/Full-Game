...
// Stub: you can later reuse your existing players.js logic.
// For now, generate random rosters with Offense/Defense players and contracts.
...
function fv2GenerateRandomPlayer(idSeed, side) {
  ...
  const salary = Math.round((ovr - 60) * 0.4) / 1;     // rough scaling in $M
...
function fv2GenerateRandomRoster() {
  const roster = [];
  let pid = 1;
  // Simple roster: 10 offense, 10 defense
  for (let i = 0; i < 10; i++) {
    roster.push(fv2GenerateRandomPlayer(pid++, "Offense"));
  }
  for (let i = 0; i < 10; i++) {
    roster.push(fv2GenerateRandomPlayer(pid++, "Defense"));
  }
  return roster;
}
...
// Generate 32-team league, 2 conferences (A/B), 4 divisions each.
function fv2GenerateLeague() {
  ...
  // Ensure 8 per region, split 4/4 into conferences.
...
// Simple round-robin-ish schedule: shuffle teams and pair them each week.
// Not NFL-accurate, but fixed and repeatable within a season. [web:428]
function fv2GenerateSeasonSchedule(league) {
  const teamIds = league.map(t => t.id);
  const schedule = []; // array of weeks; each week: [{homeId, awayId}]
...
// Build matchups for a given round from seeds and winners.
function fv2BuildPlayoffMatchups(round, seeds) {
  // seeds: array [{seed, team}]
  const getTeamBySeed = (s) => {
    const x = seeds.find(o => o.seed === s);
    return x ? x.team : null;
  };
  const matchups = [];

  if (round === "WC") {
    // 2 vs 7, 3 vs 6, 4 vs 5
    const p1 = getTeamBySeed(2), p2 = getTeamBySeed(7);
    const p3 = getTeamBySeed(3), p4 = getTeamBySeed(6);
    const p5 = getTeamBySeed(4), p6 = getTeamBySeed(5);
    if (p1 && p2) matchups.push({ homeId: p1.id, awayId: p2.id });
    if (p3 && p4) matchups.push({ homeId: p3.id, awayId: p4.id });
    if (p5 && p6) matchups.push({ homeId: p5.id, awayId: p6.id });
  } else if (round === "DIV") {
    // winners from WC vs seed 1 and vs each other; logic will consume a winners list.
  }
  // For DIV/CONF/CHAMP we will build from winners arrays in the state.
...
function renderAppV2() {
  ...
  const schedule = fv2GenerateSeasonSchedule(league);
  ...
  function fv2SimGame(homeTeam, awayTeam, isPlayoff, roundLabel) {
    ...
    return {
      ...
      loserId: homeWon ? awayTeam.id : homeTeam.id
    };
  }
  ...
  // Very simple multi-round playoff sim: 7 seeds per conference, no reseeding beyond first round.
  function simulatePlayoffRound() {
    ...
    function seedsToTeam(seedObj) { return seedObj ? seedObj.team : null; }

    if (playoffRound === "WC") {
      // Build WC matchups directly from seeds.
      function runWC(seeds, winnersOut) {
        const s2 = seedsToTeam(seeds.find(s => s.seed === 2));
        const s7 = seedsToTeam(seeds.find(s => s.seed === 7));
        const s3 = seedsToTeam(seeds.find(s => s.seed === 3));
        const s6 = seedsToTeam(seeds.find(s => s.seed === 6));
        const s4 = seedsToTeam(seeds.find(s => s.seed === 4));
        const s5 = seedsToTeam(seeds.find(s => s.seed === 5));
        const pairs = [];
      ...
      function runDIV(seeds, wcWinners, winnersOut) {
        winnersOut.length = 0;
        const s1 = seedsToTeam(seeds.find(s => s.seed === 1));
        if (!s1 || wcWinners.length < 3) return;
        // Simplified: s1 vs first WC winner, other two WC winners play each other.
      ...
      if (confWinnersA.length && confWinnersB.length) {
        ...
        fv2SimGame(tA, tB, true, roundLabel); // championship game
      }
      // Playoffs done.
      phase = "FA_OFFERS";
      playoffRound = null;
  ...
  function goToMainFranchise() {
    ...
    advanceWeekBtn.addEventListener("click", () => {
      if (phase === "REGULAR") {
        ...
        // For now, draft is just informational; then start new season.
        currentWeek = 1;
        league.forEach(t => {
          t.record.wins = 0;
          t.record.losses = 0;
